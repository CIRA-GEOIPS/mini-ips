...
def valid_plugin_data():
    ...
    return {
        ...
        # mini-ips: upstream names `read_test_v1` here, which the fork excludes
        # because it reads through `abi_netcdf` (scipy). The model validates
        # this name against the workflows registry, so it must be a workflow the
        # fork actually ships -- and under the `required` budget tier that is
        # exactly one: the fork-owned `mini_ips_reference`, built only from
        # plugins the budget guarantees.
