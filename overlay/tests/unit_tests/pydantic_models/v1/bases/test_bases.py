...
def test_good_core_base_model_model_name():
    ...
    test_model = MockCoreBaseModel(plugin_type="Reader", pluginname="geoips_netcdf")
...
def test_good_core_base_model_whitespace():
    ...
    test_model = MockCoreBaseModel(
        plugin_type="  Reader  ", plugin_name="  geoips_netcdf  "
    )
    assert test_model.plugin_type == "Reader"
    assert test_model.plugin_name == "geoips_netcdf"
...
def test_good_core_base_model_str():
    ...
    test_model = MockCoreBaseModel(plugin_type="Reader", plugin_name="geoips_netcdf")

    string_representation_of_model = str(test_model)
    expected_json_format = json.dumps(
        {"plugin_type": "Reader", "plugin_name": "geoips_netcdf"}, indent=2
    )
...
@pytest.mark.parametrize(
    "api_version",
    ["geoips/v1", "geoips_driver/v0.3", "something/v1.0"],
    ids=["v1", "geoips-driver", "non-geoips-version"],
)
